...
void MoneyBot::insert(float openingPrice, float closingPrice) {
    ...
    //inserts the percent change into the binary heap to keep a log
    plusMinusData.push(d);
...
void MoneyBot::doWeBuy() {
    ...
    while (!plusMinusData.empty())
    {
        finalSumOfDailyPlusMinus = finalSumOfDailyPlusMinus + plusMinusData.top();
        plusMinusData.pop();
    }
